...
const ListRepartosAdmin = ({ navigation }: Props) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const packagesDelivered = await handlePackagesDeliveredUser(userId);
				const updatedUser = {
					...userData,
	...
	const toggleDropdown = () => {
		...
		let toValue = dropdown
			? dropdownTitle
				? medida
				: 40
			: user.packagesDelivered.length >= 4
			? dropdownTitle
				? 282
				: 40
			: dropdownTitle
	...
	return (
		<>
			<Animated.View
				style={
					dropdownTitle && user.packagesDelivered.length >= 4
						? {
								marginTop: 10,
				...
				{showContent && dropdownTitle && (
					<>
						...
						>
							...
							>
								{user.packagesDelivered.length} Paquetes entregados
							</Text>
						</View>
						...
						>
							{dropdown
								? user.packagesDelivered.map((item, index) => (
										<View
								...
								: user.packagesDelivered.slice(0, 3).map((item, index) => (
										<View
											key={keyExtractorPackage(item, index)}
											style={{
												height: 70 * HScale,
			...
			{dropdownTitle && showContent && user.packagesDelivered.length >= 4 && (
				<Pressable
					onPress={handleArrow}
...
export default ListRepartosAdmin;
